...
public class AggregateQueryVariantSQLiteMyriaAggregate implements QueryPlanGenerator {
  ...
  public Map<Integer, RootOperator[]> getWorkerPlan(final int[] allWorkers) throws Exception {
    ...
    final DbQueryScan localScan = new DbQueryScan("select substr(sourceIPAddr, 1, 7), adRevenue from UserVisits",
        scanSchema);

    final SingleGroupByAggregate localAgg = new SingleGroupByAggregate(localScan, 0, new SingleColumnAggregatorFactory(
        1, AggregationOp.SUM));
    ...
    final GenericShuffleProducer shuffleLocalGroupBy = new GenericShuffleProducer(localAgg, shuffleLocalGroupByID,
        allWorkers, pf);
    final GenericShuffleConsumer sc = new GenericShuffleConsumer(shuffleLocalGroupBy.getSchema(), shuffleLocalGroupByID,
        allWorkers);

    final SingleGroupByAggregate globalAgg = new SingleGroupByAggregate(sc, 0, new SingleColumnAggregatorFactory(1,
        AggregationOp.SUM));
