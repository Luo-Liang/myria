...
public class Q5A_Count implements QueryPlanGenerator {
  ...
  public Map<Integer, RootOperator[]> getWorkerPlan(final int[] allWorkers) throws Exception {
    ...
    final DbQueryScan allHasCreator = new DbQueryScan(
    ...
    final GenericShuffleProducer shuffleCreatorsP = new GenericShuffleProducer(allHasCreator, allCreatorsShuffleID,
        allWorkers, pfOn0);
    final GenericShuffleConsumer shuffleCreatorsC = new GenericShuffleConsumer(shuffleCreatorsP.getSchema(),
        allCreatorsShuffleID, allWorkers);
    // schema: (createdObjID long, creatorID long)

    final SymmetricHashJoin joinArticleCreator = new SymmetricHashJoin(shuffleArticlesC, shuffleCreatorsC, new int[] {
        0 }, new int[] { 0 });
    ...
    final GenericShuffleProducer shuffleArticleCreatorsP = new GenericShuffleProducer(deArticleAuthors,
        articleCreatorsShuffleID, allWorkers, pfOn0);
    final GenericShuffleConsumer shuffleArticleCreatorsC = new GenericShuffleConsumer(shuffleArticleCreatorsP
        .getSchema(), articleCreatorsShuffleID, allWorkers);
    // schema: (articleAuthorIDs long)

    final StreamingStateWrapper deArticleAuthorsGlobal = new StreamingStateWrapper(shuffleArticleCreatorsC,
        new DupElim()); // local
    // dupelim
    // schema: (articleAuthorIDs long)

    final DbQueryScan allInProceedings = new DbQueryScan(
    ...
    final GenericShuffleProducer shuffleProceedingsP = new GenericShuffleProducer(allInProceedings,
        allProceedingsShuffleID, allWorkers, pfOn0);
    final GenericShuffleConsumer shuffleProceedingsC = new GenericShuffleConsumer(shuffleProceedingsP.getSchema(),
        allProceedingsShuffleID, allWorkers);
    // schema: (proceedingId long)

    final DbQueryScan allHasCreator2 = new DbQueryScan(
        "select t.subject, t.object from Triples t, Dictionary dtype where t.predicate=dtype.id and dtype.val='dc:creator';",
        Schemas.subjectObjectSchema);
    // schema: (createdObjectID long, creatorId long), card: 10626906

    final GenericShuffleProducer shuffleCreators2P = new GenericShuffleProducer(allHasCreator2, allCreators2ShuffleID,
        allWorkers, pfOn0);
    final GenericShuffleConsumer shuffleCreators2C = new GenericShuffleConsumer(shuffleCreators2P.getSchema(),
        allCreators2ShuffleID, allWorkers);

    final SymmetricHashJoin joinProceedingsCreator = new SymmetricHashJoin(shuffleProceedingsC, shuffleCreators2C,
        new int[] { 0 }, new int[] { 0 });
    ...
    final GenericShuffleProducer shuffleProceedingsCreatorsP = new GenericShuffleProducer(deProceedingAuthors,
        proceedingsCreatorsShuffleID, allWorkers, pfOn0);
    final GenericShuffleConsumer shuffleProceedingsCreatorsC = new GenericShuffleConsumer(shuffleProceedingsCreatorsP
        .getSchema(), proceedingsCreatorsShuffleID, allWorkers);
    // schema: (proceedingAuthorID long)

    final StreamingStateWrapper deProceedingAuthorsGlobal = new StreamingStateWrapper(shuffleProceedingsCreatorsC,
        new DupElim());

    final SymmetricHashJoin articleProceedingsCreatorJoin = new SymmetricHashJoin(deArticleAuthorsGlobal,
        deProceedingAuthorsGlobal, new int[] { 0 }, new int[] { 0 });
    // schema: (articleProceedingAuthorID long, articleProceedingAuthorID long)

    final DbQueryScan allFOAF2 = new DbQueryScan(
    ...
    final GenericShuffleProducer shuffleFOAF2P = new GenericShuffleProducer(allFOAF2, allFOAF2ShuffleID, allWorkers,
        pfOn0);
    final GenericShuffleConsumer shuffleFOAF2C = new GenericShuffleConsumer(shuffleFOAF2P.getSchema(),
        allFOAF2ShuffleID, allWorkers);
    // schema: (foafSubjID long, foafObjID long)

    final SymmetricHashJoin articleProceedingsCreatorFOAFJoin = new SymmetricHashJoin(articleProceedingsCreatorJoin,
        shuffleFOAF2C, new int[] { 1 }, new int[] { 0 });
    ...
    final GenericShuffleProducer forDupElimShuffleP = new GenericShuffleProducer(finalColSelect, forDupElimShuffleID,
        allWorkers, new MultiFieldHashPartitionFunction(allWorkers.length, new int[] { 0, 1 }));
