...
public class Q9 implements QueryPlanGenerator {
  ...
  public Map<Integer, RootOperator[]> getWorkerPlan(final int[] allWorkers) throws Exception {
    ...
    final GenericShuffleProducer shuffleAllPersonsP = new GenericShuffleProducer(allPersons, allPersonsShuffleID,
        allWorkers, pfOn0);
    final GenericShuffleConsumer shuffleAllPersonsC = new GenericShuffleConsumer(shuffleAllPersonsP.getSchema(),
        allPersonsShuffleID, allWorkers);
    // schema: (personID long)

    final LocalMultiwayProducer multiPersonProducer = new LocalMultiwayProducer(shuffleAllPersonsC, ImmutableList.of(
        allPersonsOutLocalMultiWayID, allPersonsInLocalMultiWayID));
    final LocalMultiwayConsumer multiPersonInConsumer = new LocalMultiwayConsumer(multiPersonProducer.getSchema(),
        allPersonsInLocalMultiWayID);
    final LocalMultiwayConsumer multiPersonOutConsumer = new LocalMultiwayConsumer(multiPersonProducer.getSchema(),
        allPersonsOutLocalMultiWayID);
    ...
    final DbQueryScan allTriples = new DbQueryScan(
        "SELECT t.subject,d.val,t.object FROM Triples t,Dictionary d where t.predicate=d.ID", triplesSchema);
    // schema: (subject long, predicateName string, object long)

    final LocalMultiwayProducer multiTriplesProducer = new LocalMultiwayProducer(allTriples, ImmutableList.of(
        allTriplesOutLocalMultiWayID, allTriplesInLocalMultiWayID));
    final LocalMultiwayConsumer multiTriplesInConsumer = new LocalMultiwayConsumer(multiTriplesProducer.getSchema(),
        allTriplesInLocalMultiWayID);
    final LocalMultiwayConsumer multiTriplesOutConsumer = new LocalMultiwayConsumer(multiTriplesProducer.getSchema(),
        allTriplesOutLocalMultiWayID);

    final SymmetricHashJoin joinPersonsTriplesIn = new SymmetricHashJoin(multiPersonInConsumer, multiTriplesInConsumer,
        new int[] { 0 }, new int[] { 2 });
    ...
    final SymmetricHashJoin joinPersonsTriplesOut = new SymmetricHashJoin(multiPersonOutConsumer,
        multiTriplesOutConsumer, new int[] { 0 }, new int[] { 0 });
