...
public class SingleGroupByAggregate extends UnaryOperator {
    ...
    /**
     * If Sketch is enabled, save tuple as key.
     */
    ...
    protected final void init(final ImmutableMap<String, Object> execEnvVars) throws DbException {
        ...
        boolean anyPreciseAggregates = Arrays.stream(aggregators).anyMatch(o -> (o instanceof IntegerAggregator) == false || ((IntegerAggregator) o).getSketchOption() == AggregationSketchOption.DoNotSketch);
        boolean anySketchAggregates = Arrays.stream(aggregators).anyMatch(o -> (o instanceof IntegerAggregator) && ((IntegerAggregator) o).getSketchOption() != AggregationSketchOption.DoNotSketch);
        ...
        if (sketchEnabled && anySketchAggregates) {
            ...
            for (int i = 0; i < aggregators.length; i++) {
                sketchBuffers[i] = new SketchBuffer(SketchBuffer.DEFAULT_ROWS, SketchBuffer.DEFAULT_COLUMN, aggregators[i]);
            }
            sketchGroupKeys = new HashSet<>();
        } else {
            switch (gColumnType) {
                case BOOLEAN_TYPE:
