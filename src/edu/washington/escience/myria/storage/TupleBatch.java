...
public class TupleBatch implements ReadableTable, Serializable {
  ...
  public final TupleBatch[] partition(final PartitionFunction pf) {
    TupleBatch[] result = new TupleBatch[pf.numDestinations()];
    if (isEOI) {
      Arrays.fill(result, this);
      return result;
    }
    BitSet[] masks = new BitSet[pf.numDestinations()];
    for (int i = 0; i < masks.length; ++i) {
      masks[i] = new BitSet(numTuples);
    }
    for (int i = 0; i < numTuples(); ++i) {
      int[] ps = pf.distribute(this, i);
      for (int p : ps) {
        masks[p].set(i);
      }
    }
    for (int i = 0; i < result.length; ++i) {
      result[i] = filter(masks[i]);
    }
    return result;
  }
